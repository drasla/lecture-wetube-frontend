...
export default function VideoManage() {
    ...
    const handleDelete = async (id: number, title: string) => {
        if (!window.confirm(`'${title}' 영상을 정말 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.`)) return;
        try {
    ...
    return (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-gray-200 flex justify-between items-center">
                ...
                <span className="text-sm text-gray-500">총 {videos.length}개 (현재 페이지)</span>
            </div>
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <div className="min-w-0">
                                        <Link
                                            ...
                                            className="font-semibold text-gray-800 hover:text-blue-600 truncate block flex items-center gap-1"
                                        >
                                            {video.title} <MdOpenInNew className="text-gray-400 text-xs"/>
                                        </Link>
                                        <p className="text-xs text-gray-500 line-clamp-1">{video.description || "설명 없음"}</p>
                                    </div>
                                </div>
                            </td>
                            <td className="px-6 py-4">
                                <div className="flex flex-col">
                                    <span className="font-medium text-gray-800">{video.author.nickname}</span>
                                    <span className="text-xs text-gray-400">{video.author.email}</span>
                                </div>
                            </td>
                            <td className="px-6 py-4 text-center">
                                <div className="text-xs">
                                    <span className="font-bold text-gray-700">{video.views.toLocaleString()}</span> views
                                    <br />
                                    <span className="text-red-500">♥ {video._count.likes}</span>
                                </div>
                            </td>
                            <td className="px-6 py-4 text-center">
                                {dayjs(video.createdAt).format("YYYY-MM-DD")}
                                <br />
                                <span className="text-xs text-gray-400">{dayjs(video.createdAt).format("HH:mm")}</span>
                            </td>
                            <td className="px-6 py-4 text-center">
                                <button
